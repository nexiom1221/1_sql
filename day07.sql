...
SELECT to_char(12345,'9999')  --12345 
  FROM dual
;
SELECT to_char(12345,'99999') as "Dwad" --12345 
  FROM dual
;
SELECT 12345
  FROM dual
...
SELECT to_char(12345,'9999999') as "데이터"  --12345 
  FROM dual
;
--앞에 빈 공간을 0으로 채우기, 뒤에 소수점
SELECT to_char(12345,'9999999.99') as 데이터  --12345 
  FROM dual
;
...
입력결과
-------------------------------
입력값이 YES/NO 중 어느것도 아님
*/

-- emp 테이블에서 job 별로 경조사비를 급여대비 일정 비율로 지급하기로 했다.
--지급비율이 다음과 같다고 할때,
...
SELECT   e.empno
        ...
        ,to_char(DECODE(e.job,'CLERK'        , e.sal * 0.05
                             ...
                             ,'MANAGER'      , e.sal * 0.037
                             ,'ANALYST'      , e.sal * 0.03
                             ,'PRESIDENT'    , e.sal * 0.015),'$999.99') as "경조사 지원금"
  FROM emp e
;
/*
EMPNO, ENAME, JOB, 경조사 지원금
